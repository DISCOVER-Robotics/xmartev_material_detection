...
cfg.gs_model_dict = {
    ...
    "yellowbox": os.path.join(ply_dir, "xmartev_material_detection", "yellowbox.ply"),
    "pinkbox": os.path.join(ply_dir, "xmartev_material_detection", "pinkbox.ply"),
...
class SceneROS2Node(MMK2ROS2):
    def __init__(self, config, random):
        ...
        self.random = random
        # new
        # self.referee = Referee(self.mj_model, os.path.join(current_dir, "referee_json/r1_rules.json"))   
        json_name = f"task_2_{random+1}_rules.json"
        json_dir = f"{current_dir}/referee_json/{json_name}"
        # print(json_dir)                 
    ...
    def domain_randomization(self):
        ...
        height_choice = [0.568601, 0.891876, 1.2228054]
    ...
    def reset(self):
        ...
        if self.random == 0:
            task_info_msg.data = f"{r}: Take the pinkbox from the cabinet, and put it on the right side of the drill."
        if self.random == 1:
...
def random_scene(random):
    if random == 0:
        cfg.mjcf_file_path = os.path.join(models_dir, "mjcf/xmartev_task_2_1.xml")
        cfg.obj_list.extend([ "toolbox",  "drill"])
        cfg.gs_model_dict.update({
        "toolbox": os.path.join(ply_dir, "xmartev_material_detection", "toolbox.ply"),
        "drill": os.path.join(ply_dir, "xmartev_material_detection", "drill.ply")
        })

    if random == 1:
        cfg.mjcf_file_path = os.path.join(models_dir, "mjcf/xmartev_task_2_2.xml")
        cfg.obj_list.extend(["hat", "brownbox", "yellowbox","pinkbox", "blue"])
        cfg.gs_model_dict.update({
        "hat": os.path.join(ply_dir, "xmartev_material_detection", "hat.ply"),
        "blue": os.path.join(ply_dir, "xmartev_material_detection", "blue.ply")
        })

    if random == 2:
        cfg.mjcf_file_path = os.path.join(models_dir, "mjcf/xmartev_task_2_3.xml")
        cfg.obj_list.extend(["toolbox", "brownbox", "yellowbox","pinkbox", "blue",])
        cfg.gs_model_dict.update({
        "toolbox": os.path.join(ply_dir, "xmartev_material_detection", "toolbox.ply"),
        "blue": os.path.join(ply_dir, "xmartev_material_detection", "blue.ply")
        })
...
if __name__ == "__main__":
    ...
    # 简化参数解析（仅保留必要配置）
    parser = argparse.ArgumentParser(description='ROS2 Scene Server for xmartev_material_detection task_2 Environment')
    ...
    # 初始化场景节点
    sim_node = SceneROS2Node(cfg,random)
    obs = sim_node.reset()
